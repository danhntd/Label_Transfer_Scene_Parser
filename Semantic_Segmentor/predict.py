...
def parse_args():
    ...
    parser.add_argument('--path_to_save', dest='path_to_save',
                          help='path of the folder to save inference',
                          type=str)
    parser.add_argument('--path_to_test_set', dest='path_to_test_set',
                          help='path of the test set folder, with provided folder structure',
                          type=str)
...
def main():
    ...
    path_gt_to_save = path_to_save + "GT"
    path_img_to_save = path_to_save + "Image"
    
    path_to_test_set = args.path_to_test_set
    path_img_to_infer = args.path_to_test_set + "/leftImg8bit/test/night/"
    path_to_gtFine = args.path_to_test_set + "/gtFine_trainvaltest/gtFine/test/night/"
    ...
    for image_name in tqdm(images_list):
        ...
        cv2.imwrite(os.path.join(path_img_to_save, str("{:06d}_".format(idx) + "image_" + image_name)), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
        
        #save gt in same format name
        gtFine_path = os.path.join(path_to_gtFine, image_name.replace("leftImg8bit", "gtFine_color"))
        gtFine = scipy.misc.imread(gtFine_path, mode='RGB')
        cv2.imwrite(os.path.join(path_img_to_save, str("{:06d}_".format(idx) + "gtFine_" + image_name)), cv2.cvtColor(gtFine, cv2.COLOR_RGB2BGR))
        ...
        scipy.misc.imsave(os.path.join(path_img_to_save, str("{:06d}_".format(idx) + "pred_color_" + image_name)), pred_rgb)
        
        pred = pred.transpose(1,2,0)
        cv2.imwrite(os.path.join(path_gt_to_save, str("{:06d}_".format(idx) + "pred_trainIds_" + image_name)), pred)
        ...
        cv2.imwrite(os.path.join(path_gt_to_save, str("{:06d}_".format(idx) + "pred_labelIds_" + image_name)), pred)
        idx+=1
